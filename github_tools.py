...
def get_deployment_by_service_region(service: str, region: str = None, hours: int = 24) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for deployment in deployments:
            ...
            service_match = (
                service.lower() in deployment.get('environment', '').lower() or
                service.lower() in deployment.get('description', '').lower() or
                service.lower() in deployment.get('ref', '').lower()
            )
            ...
            if service_match:
                ...
                if region:
                    region_match = (
                        region.lower() in deployment.get('environment', '').lower() or
                        region.lower() in deployment.get('description', '').lower()
                    )
                    if region_match:
